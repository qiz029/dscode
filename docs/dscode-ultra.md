...
The parent can pass `reasoning_effort` to `subagent` or `subagent_fork`, independently of the model-selection opt-in. For example: `{"description":"Add focused unit tests","prompt":"...","reasoning_effort":"low"}`. Omission keeps existing inheritance. The selected child model validates the effort before child creation; unsupported values fail instead of silently falling back. Effort-only selection does not change provider/model or the parent's effort. Explicit provider/model overrides still require the existing model-selection policy. Ultra guidance asks parents to choose low for bounded work, high for difficult work and max only when justified. This configures newly created children, not existing children sent another message.

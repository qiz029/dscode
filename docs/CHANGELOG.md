...
- Fresh Hub installs no longer reject the exact empty `cordis.yml` generated by Harness composition as a dependency-input mutation. The launcher vendors Hub CLI 0.5.0 with a narrow content-hash exception; custom root content, package-manager hooks, other configuration and dependency locks retain their checks.
- The release gate now requires the native locked installer, launcher first start and Hub doctor in addition to the existing installed-agent and rollback probes. The earlier external-executor fixture did not exercise native dependency receipts.

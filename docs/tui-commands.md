...
| `/mouse` | Toggle mouse capture. Off (default) keeps the terminal's normal drag-select and copy, with PageUp/PageDown scrolling the chat; on makes the wheel scroll the chat (hold Option in iTerm2 or Fn in Terminal.app to select while it is on). The choice is saved per machine in `~/.dsh/dsh-code/mouse.json`. |

...
| `/provider [deepseek\|openrouter]` | Switch the session between DeepSeek's official API and OpenRouter. Without an argument, open a picker (↑↓ choose, Enter switch, Esc close) that shows each provider's key status. The first OpenRouter switch declares the `openrouter` route in `$DSH_HOME/settings.yaml` with DeepSeek V4 Flash, V4 Pro and V4 Flash Vision Exp (an existing `openrouter` profile is left alone; edit it later through `/model` → `a`), asks for the key when none is stored, then moves to the counterpart of the current model and keeps the effort when the target offers it. Ultra works on both routes: on OpenRouter it sends `xhigh` plus the collaboration policy, and `low` is sent as `high`, which is how DeepSeek itself answers low. The footer shows the active provider's remaining balance (for OpenRouter, credits minus usage) and estimates OpenRouter cost from list prices; the 🔥/❄️ peak marker appears only for DeepSeek, because OpenRouter bills no peak window. Stop a running turn first. |

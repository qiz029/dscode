...
交付物是可安装的 `dscode-0.7.4.tar.gz`，不是只有配置的 `.dshprofile`。安装后用户直接运行 `dscode`。npm + Hub 安装已提供，tar 包通过 GitHub Releases 分发；尚无 Homebrew formula。
...
分发 `artifacts/dscode-0.7.4.tar.gz`。包里包含启动器、preset、依赖 manifest 和完整 lockfile，不含模型密钥、本地覆盖配置、会话、node_modules 或研究文件。安装时从 npm 下载锁定依赖，需要联网。

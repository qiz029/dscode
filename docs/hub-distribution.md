...
`verify:hub` installs a real launcher in a temporary directory and serves the not-yet-published bundle from a loopback npm registry. It checks the native locked Hub installer, launcher first start and Hub doctor, then uses separate external-executor fixtures for the agent loop, replacement and rollback. Publication requires both sets of results against the exact tarball hashes. It is not equivalent to public Hub discovery or a real npm publish. The tests request no remote model.

The launcher vendors Hub CLI 0.5.0 with a narrow compatibility fix for the empty `cordis.yml` generated by Harness composition. Only the exact generated content is excluded from dependency-input receipts, and only when package-manager hooks are absent. Custom root content, other configuration and dependency locks remain checked. The compatibility fix is applied to build copies; recipient dependencies are not patched during installation.

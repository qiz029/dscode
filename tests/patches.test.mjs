...
test('pristine locked upstream accepts all patches once, remains valid JS, and is idempotent', t => {
  ...
  for (const marker of ['label: "/verbose"', 'if (text === "/verbose")', 'label: "/mouse"', 'if (text === "/mouse")', 'function dscodeSetMouse(', 'notify(dscodeT(dscodeSetMouse(!dscodeMouseEnabled)', 'label: "/language"', 'if (text === "/language"', 'function dscodeT(', 'dscodeT("activity.running")', 'dscodeT("welcome.model")', 'notify(dscodeT(next ? "verbose.on" : "verbose.off"))']) assert(tui.includes(marker), marker);

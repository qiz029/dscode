export function summarize(rows) {
  ...
  for (const row of rows) {
    ...
    if (!group) groups.set(row.policy, group = { policy: row.policy, checkpoints: 0, errors: 0, formatDeviations: 0, passed: 0, total: 0, ungraded: 0, compactions: 0, prunes: 0, estimatedBeforeTokens: 0, estimatedAfterTokens: 0, categories: {}, cases: new Set() });
...
export function markdownReport(manifest, rows, calls) {
  ...
  return [
    ...
    '| Policy | Correct / probes | Ungraded | Eval errors | Fenced JSON | Summaries | Tool prunes | Compression exercised |',
    '|---|---:|---:|---:|---:|---:|---:|---|',
    ...groups.map(group => `| ${group.policy} | ${group.passed} / ${group.total} | ${group.ungraded} | ${group.errors} | ${group.formatDeviations} | ${group.compactions} | ${group.prunes} | ${group.exercised ? 'yes' : 'no'} |`), '',
